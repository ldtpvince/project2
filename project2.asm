...
check_Logic:
	...
	bne	$t0,$zero,return_cl
	addi	$a0,$a2,0
	jal	getDate	#Lay so ngay thang $a1, nam $a0
	lw	$a0,0($sp)
	addi	$v0,$v0,1
	slt	$t0,$a0,$v0
	addi	$v0,$zero,0	#False
...
getDate:
	...
	sw	$a1,4($sp)
	sw	$ra,8($sp)
	#Neu la nam nhuan va la thang 2 thi toi notLeap
	jal	LeapYearNumber
	beq	$v0,$zero,notLeap
	bne	$a1,2,notLeap
	addi	$v0,$zero,29
...
std_Str:
	subi	$sp,$sp,8
	sw	$ra,0($sp)
	#$t0 chua dia chi cuoi string cap phat
	addi	$t0,$a0,0
	add	$t0,$t0,$a1
	subi	$t0,$t0,1
	sb	$zero,0($t0)
	#$t1 chua di chi cuoi string (ket thuc '\n' hoac '\0')
	sw	$t0,4($sp)
	jal	findEndStr
	lw	$t0,4($sp)
	addi	$t1,$v0,0
...
Convert: 
	...
	sw	$a0, 16($sp)
	sw	$a1, 20($sp)
	#Lay ngay, thang, nam lưu vào stack
	jal	Day
	sw	$v0,4($sp)
	jal	Month
	sw	$v0,8($sp)
